...
void WIFI_SETUP() {
  ...
  if (testTime == 0) {                      // If time text test mode is NOT used:
    ...
    if (WiFiConfigEmpty == true) {
      ...
      if (getSetting(setting_type::useStartupText) == 1) callStartText();  // Show "WordClock" startup text
      if (getSetting(setting_type::useshowip) == 1) ShowIPaddress();       // Display the current IP-address
